/* Capyt · Planung -> Capyt · Mobile FP1 integration addon.
...
(() => {
  ...
  function fp1Meta() {
    ...
    if (!isPlainObject(m.rejectedTransactions)) m.rejectedTransactions = {};
  ...
  function fp1ConvertDonut(data, mode) {
    return { mode, title:String(data.title||''),centerLabel:String(data.centerLabel||''),centerSubtext:String(data.centerSubtext||''),totalCents:fp1EuroToCents(data.total),segments:(data.segments||[]).map(s=>({key:String(s.key),label:String(s.label),amountCents:fp1EuroToCents(s.amount),group:String(s.group||'cost'),color:String(s.color||getComputedStyle(document.documentElement).getPropertyValue('--color-primary').trim()),details:(s.details||[]).map(d=>({label:String(d.label||s.label),amountCents:fp1EuroToCents(d.amount)}))})) };
  }
  ...
  function fp1PlanPayload(ym) {
    ...
    return { protocolVersion:1,type:'P',planId:meta.planId,planName:'Capyt · Finanzplanung',revision:meta.revision,month:ym,createdAt:new Date().toISOString(),source:{app:'Capyt · Planung',dataVersion:CURRENT_DATA_VERSION},accountBalanceCents:result.actualBankBalance==null?null:fp1EuroToCents(result.actualBankBalance),normalBalanceCents:fp1EuroToCents(result.effectiveEndingBalance),freeAvailableCents:fp1EuroToCents(result.freeAvailable),minimumCashBufferCents:fp1EuroToCents(result.minimumCashBuffer),budgetAssetsCents:fp1EuroToCents(result.budgetClosingTotal),savingsAssetsCents:fp1EuroToCents(result.savingsAssetsTotal),totalAssetsCents:fp1EuroToCents(result.totalAssets),budgets,savingsGoals:goals,donuts:{planned:fp1ConvertDonut(buildMonthAllocationData(ym,result),'planned'),actual:fp1ConvertDonut(buildMonthExpenseData(ym,result),'actual')},acknowledgedTransactions:ackTx,acknowledgedTransactionIds:[],acknowledgedExportIds:meta.acknowledgedExportIds.slice(-500),transactionResults,lastSync:{transactionImportAt:meta.lastTransactionImportAt},capy:globalThis.CapytCapyDesktopBridge?.syncSnapshot(ym)||{enabled:false,budgetId:'',budgetName:'',stashBalanceCents:0,withdrawableStashCents:0,lockedStashCents:0,nextUnlockDate:'',stashLockMonths:1,coins:0,acknowledgedCoinOpIds:[],care:null}};
  }
  ...
  function fp1Classification(payload) {
    ...
    return payload.transactions.map(raw=>{
      try{
        ...
        if(tx.op!=='delete'&&['budget_expense','capy_stash_deposit'].includes(tx.kind)&&!fp1FindBudget(tx.budgetId))return {tx,status:'error',message:'Budget-ID ist am Desktop nicht vorhanden.'};
  ...
  function fp1ReverseImported(record){ if(!record||record.deleted)return;const tx=record.data,monthData=state.months[tx.month];if(!monthData)return;if(tx.kind==='budget_expense'){monthData.budgetUsage[tx.budgetId]=roundMoney(Math.max(0,asNumber(monthData.budgetUsage[tx.budgetId])-fp1CentsToEuro(tx.amountCents)));}
    else if(tx.kind==='capy_stash_deposit'){monthData.budgetTransactions=(monthData.budgetTransactions||[]).filter(x=>String(x.mobileTransactionId||'')!==String(tx.id));}
  ...
  function fp1ApplyOne(tx){const meta=fp1Meta(),previous=meta.importedTransactions[tx.id];if(previous)fp1ReverseImported(previous);const monthData=state.months[tx.month];
    if(tx.op!=='delete'){
      ...
      else if(tx.kind==='capy_stash_deposit'){monthData.budgetTransactions=monthData.budgetTransactions||[];monthData.budgetTransactions.push({id:`mobile-${tx.id}`,mobileTransactionId:tx.id,mobileRecordRevision:tx.recordRevision,type:'cash_to_budget',budgetId:tx.budgetId,fromBudgetId:'',toBudgetId:tx.budgetId,amount:fp1CentsToEuro(tx.amountCents),date:tx.date,note:tx.note||tx.description||'Capy Vorrat mobil aufgeladen',capyUnlockDate:globalThis.CapytCapyDesktopBridge?.lockDateFor?.(tx.date)||''});}
  ...
  function fp1ApplyCapy(payload){return globalThis.CapytCapyDesktopBridge?.applyMobileSync(payload)||{coinOps:0,care:false};}
  function fp1CapyForRows(capy,rows){if(!capy)return null;const blockedTransactionIds=new Set((rows||[]).filter(r=>r.status==='error').map(r=>String(r.tx?.id||'')));return {...capy,coinOps:(capy.coinOps||[]).filter(op=>!op.relatedTransactionId||!blockedTransactionIds.has(String(op.relatedTransactionId)))};}
  ...
  async function fp1PreviewTransactionCode(){
    ...
    const capyNote=payload.capy?'<br>Capy-Daten (Pflege/Coins) sind im Code enthalten.':'';
    document.getElementById('fp1ImportSummary').innerHTML=`<div class="notice ${counts.error?'warn':''}"><strong>${rows.length} Buchungsänderung${rows.length===1?'':'en'} gefunden${payload.capy?' + Capy-Daten':''}</strong><br>${counts.new} neu · ${counts.update} geändert · ${counts.duplicate} bereits vorhanden · ${counts.error} Fehler${capyNote}${stale?'<br>Hinweis: Der Mobile-Code basiert auf einem älteren Planstand. Tatsächliche Buchungen bleiben trotzdem importierbar.':''}</div>${rows.length?`<div class="table-wrap"><table><thead><tr><th>Status</th><th>Buchung</th><th>Betrag</th><th>Hinweis</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${fp1Escape(r.status)}</td><td>${fp1Escape(r.tx.description||r.tx.category||r.tx.id)}</td><td class="num">${r.tx.amountCents?money(fp1CentsToEuro(r.tx.amountCents)):'–'}</td><td>${fp1Escape(r.message)}</td></tr>`).join('')}</tbody></table></div>`:''}<button id="fp1ApplyImport" type="button" class="primary" style="margin-top:.7rem">Gültige Änderungen übernehmen</button>`;
    document.getElementById('fp1ApplyImport').addEventListener('click',fp1ApplyImport);
  }
  function fp1ApplyImport(){if(!fp1PendingImport)return;const {payload,rows}=fp1PendingImport;const capyResult=fp1ApplyCapy(fp1CapyForRows(payload.capy,rows));let applied=0;for(const row of rows){if(row.status==='new'||row.status==='update'){fp1ApplyOne(row.tx);applied++;}}
    const meta=fp1Meta(),nowIso=new Date().toISOString();for(const row of rows){const id=String(row.tx?.id||'');if(!id)continue;if(row.status==='error'){meta.rejectedTransactions[id]={id,recordRevision:Number(row.tx?.recordRevision)||1,reason:String(row.message||'Nicht übernommen'),updatedAt:nowIso};}else delete meta.rejectedTransactions[id];}
    const errors=rows.filter(r=>r.status==='error').length;if(!errors&&payload.exportId){meta.acknowledgedExportIds=[...new Set([...meta.acknowledgedExportIds,String(payload.exportId)])].slice(-500);}meta.lastTransactionImportAt=nowIso;const capyApplied=(capyResult.coinOps||0)+(capyResult.care?1:0)+(capyResult.enabled?1:0);saveState(`Mobile Sync: ${applied+capyApplied} Änderung(en) übernommen`);renderAll();document.getElementById('fp1ImportSummary').insertAdjacentHTML('afterbegin',`<div class="notice">Import abgeschlossen: ${applied} Buchungsänderung(en) übernommen${payload.capy?` · Capy: ${capyResult.coinOps||0} Coin-Operation(en)${capyResult.care?' + Pflegestand':''}${capyResult.enabled?' + Aktivierungsstatus':''}`:''}. ${rows.filter(r=>r.status==='duplicate').length} Duplikat(e) ignoriert.${errors?` ${errors} Fehler nicht übernommen und für den Rücksync markiert.`:''}</div>`);fp1PendingImport=null;}
  ...
  function fp1InjectUi(){
    ...
    const panel=document.createElement('div');panel.className='panel';panel.id='fp1DesktopSyncPanel';panel.innerHTML=`<h2>Capyt · Mobile synchronisieren</h2><p class="help">Capyt · Planung bleibt die Planungsinstanz. PLAN-CODE sendet berechnete Monatswerte an das Smartphone; TRANSAKTIONS-CODE übernimmt tatsächliche mobile Buchungen per ID und Buchungsrevision.</p><div class="grid"><div><h3>PLAN-CODE · PC → Smartphone</h3><div class="field"><label for="fp1DesktopMonth">Monat</label><select id="fp1DesktopMonth">${months.map(ym=>`<option value="${ym}" ${ym===selectedMonth?'selected':''}>${fp1Escape(monthName(ym))}</option>`).join('')}</select></div><button id="fp1GeneratePlan" type="button" class="primary" style="margin-top:.6rem">Code für Capyt · Mobile erzeugen</button><p id="fp1PlanSummary" class="help"></p><div id="fp1PlanQr"></div><p id="fp1PlanQrError" class="help"></p><textarea id="fp1PlanCode" rows="6" readonly style="width:100%;margin-top:.6rem"></textarea><button id="fp1CopyPlan" type="button" style="margin-top:.4rem">Plan-Code kopieren</button></div><div><h3>TRANSAKTIONS-CODE · Smartphone → PC</h3><textarea id="fp1TransactionCode" rows="7" placeholder="FP1-T-C-…" style="width:100%"></textarea><button id="fp1PreviewImport" type="button" class="primary" style="margin-top:.6rem">Code prüfen</button><div id="fp1ImportSummary"></div></div></div>`;target.prepend(panel);
  ...
  if (globalThis.__FP1_ENABLE_TEST_HOOKS__) {
    globalThis.__FP1_DESKTOP_TEST_API__ = { fp1Meta, fp1Encode, fp1Decode, fp1PlanPayload, fp1ValidateTx, fp1Classification, fp1ReverseImported, fp1ApplyOne, fp1ApplyCapy, fp1CapyForRows };
  }
