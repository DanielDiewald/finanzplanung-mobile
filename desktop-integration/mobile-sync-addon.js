/* Benedikt desktop -> mobile FP1 integration addon.
   Load after the desktop planner. Requires local qrcode.min.js before this file for QR output. */
(() => {
  'use strict';
  ...
  function fp1ConvertDonut(data, mode) {
    return { mode, title:String(data.title||''),centerLabel:String(data.centerLabel||''),centerSubtext:String(data.centerSubtext||''),totalCents:fp1EuroToCents(data.total),segments:(data.segments||[]).map(s=>({key:String(s.key),label:String(s.label),amountCents:fp1EuroToCents(s.amount),group:String(s.group||'cost'),color:String(s.color||'#4c86af'),details:(s.details||[]).map(d=>({label:String(d.label||s.label),amountCents:fp1EuroToCents(d.amount)}))})) };
  }
  ...
  function fp1PlanPayload(ym) {
    ...
    return { protocolVersion:1,type:'P',planId:meta.planId,planName:'Benedikt',revision:meta.revision,month:ym,createdAt:new Date().toISOString(),source:{app:'Benedikt Desktop',dataVersion:CURRENT_DATA_VERSION},accountBalanceCents:result.actualBankBalance==null?null:fp1EuroToCents(result.actualBankBalance),normalBalanceCents:fp1EuroToCents(result.effectiveEndingBalance),freeAvailableCents:fp1EuroToCents(result.freeAvailable),minimumCashBufferCents:fp1EuroToCents(result.minimumCashBuffer),budgetAssetsCents:fp1EuroToCents(result.budgetClosingTotal),savingsAssetsCents:fp1EuroToCents(result.savingsAssetsTotal),totalAssetsCents:fp1EuroToCents(result.totalAssets),budgets,savingsGoals:goals,donuts:{planned:fp1ConvertDonut(buildMonthAllocationData(ym,result),'planned'),actual:fp1ConvertDonut(buildMonthExpenseData(ym,result),'actual')},acknowledgedTransactions:ackTx,acknowledgedTransactionIds:[],acknowledgedExportIds:meta.acknowledgedExportIds.slice(-500),lastSync:{transactionImportAt:meta.lastTransactionImportAt}};
  }
  ...
  function fp1Classification(payload) {
    const meta=fp1Meta(); if(String(payload.planId)!==String(meta.planId))throw new Error('Plan-ID stimmt nicht mit diesem Plan überein.'); if(!Array.isArray(payload.transactions))throw new Error('Transaktionsliste fehlt.');
  ...
  function fp1InjectUi(){
    ...
    const panel=document.createElement('div');panel.className='panel';panel.id='fp1DesktopSyncPanel';panel.innerHTML=`<div class="toolbar"><div><h2 style="margin:0">Smartphone synchronisieren</h2><p class="help" style="margin:.3rem 0 0">Übertrage den aktuellen Plan auf dein Smartphone oder hole dort erfasste Buchungen zurück auf den PC.</p></div></div><div class="grid"><div class="sync-direction"><div class="sync-step"><span>1</span><div><p class="sync-kicker">PC → Smartphone</p><h3>Plan übernehmen</h3></div></div><div class="field"><label for="fp1DesktopMonth">Monat</label><select id="fp1DesktopMonth">${months.map(ym=>`<option value="${ym}" ${ym===selectedMonth?'selected':''}>${fp1Escape(monthName(ym))}</option>`).join('')}</select></div><button id="fp1GeneratePlan" type="button" class="primary" style="margin-top:.6rem">Plan-Code erzeugen</button><p id="fp1PlanSummary" class="help"></p><div id="fp1PlanQr" class="fp1-qr-card"></div><p id="fp1PlanQrError" class="help"></p><div class="field" style="margin-top:.6rem"><label for="fp1PlanCode">Textcode</label><textarea id="fp1PlanCode" rows="6" readonly></textarea></div><button id="fp1CopyPlan" type="button" style="margin-top:.4rem">Code kopieren</button></div><div class="sync-direction"><div class="sync-step"><span>2</span><div><p class="sync-kicker">Smartphone → PC</p><h3>Buchungen übertragen</h3></div></div><p class="help">Füge den auf dem Smartphone erzeugten Buchungscode ein und prüfe ihn vor der Übernahme.</p><textarea id="fp1TransactionCode" rows="7" placeholder="Buchungscode einfügen …"></textarea><button id="fp1PreviewImport" type="button" class="primary" style="margin-top:.6rem">Code prüfen</button><div id="fp1ImportSummary"></div></div></div>`;target.prepend(panel);
