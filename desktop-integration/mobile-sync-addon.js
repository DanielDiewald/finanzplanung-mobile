...
(() => {
  ...
  function fp1ApplyOne(tx){const meta=fp1Meta(),previous=meta.importedTransactions[tx.id];if(previous)fp1ReverseImported(previous);const monthData=state.months[tx.month];
    if(tx.op!=='delete'){
      ...
      else if(tx.kind==='capy_stash_deposit'){monthData.budgetTransactions=monthData.budgetTransactions||[];monthData.budgetTransactions.push({id:`mobile-${tx.id}`,mobileTransactionId:tx.id,mobileRecordRevision:tx.recordRevision,type:'cash_to_budget',budgetId:tx.budgetId,fromBudgetId:'',toBudgetId:tx.budgetId,amount:fp1CentsToEuro(tx.amountCents),date:tx.date,note:tx.note||tx.description||'Capy Vorrat mobil aufgeladen'});}
