...
(() => {
  ...
  async function fp1Encode(type,payload) {
    const raw=te.encode(fp1Canonical({...payload,protocolVersion:1,type})); let enc='N',body=raw;
    if(globalThis.CompressionStream){enc='Z';body=await fp1Transform(raw,'deflate');}
  ...
  async function fp1Decode(code,expected='T') {
    ...
    const [prefix,type,enc,checksum,...rest]=parts;if(prefix!=='FP1')throw new Error('Nicht unterstützte FP-Version.');if(type!==expected)throw new Error(`Falscher Code-Typ: FP1-${type}.`);if(!['Z','N'].includes(enc))throw new Error('Unbekannte FP1-Kodierung.');
    ...
    const raw=enc==='Z'?await fp1Transform(body,'deflate',true):body;const payload=JSON.parse(td.decode(raw));if(payload.protocolVersion!==1||payload.type!==type)throw new Error('FP1-Nutzdaten sind inkompatibel.');return payload;
  }
  ...
  async function fp1GeneratePlanCode() {
    ...
    if(typeof QRCode==='function'){try{new QRCode(qr,{text:code,width:260,height:260,correctLevel:QRCode.CorrectLevel.L});document.getElementById('fp1PlanQrError').textContent='';}catch(err){document.getElementById('fp1PlanQrError').textContent='QR zu groß; Textcode verwenden.';}}
    document.getElementById('fp1PlanSummary').textContent=`FP1-P · ${monthName(ym)} · Revision ${meta.revision} · ${payload.budgets.length} Budgets`;
  }
