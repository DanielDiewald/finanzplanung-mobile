...
function drawDonut(canvas,data,selectedKey,onSelect){
  ...
  if(!(total>0)){ ctx.strokeStyle=getComputedStyle(document.documentElement).getPropertyValue('--line').trim()||'#d6d9de'; ctx.beginPath(); ctx.arc(cx,cy,r,0,Math.PI*2); ctx.stroke(); }
    ...
    for(const segment of segments){ const span=(segment.amountCents/total)*Math.PI*2; const a0=angle+gap/2,a1=angle+span-gap/2; if(a1>a0){ ctx.strokeStyle=segment.color||'#4c86af'; ctx.globalAlpha=selectedKey&&selectedKey!==segment.key?.toString()?.trim()?0.42:1; ctx.beginPath(); ctx.arc(cx,cy,r,a0,a1); ctx.stroke(); ctx.globalAlpha=1; hits.push({key:segment.key,start:angle,end:angle+span}); } angle+=span; }
  }
...
export function renderMonth({display,donutMode='planned',selectedSegment='',onSegment,onBudget}){
  ...
  document.getElementById('freeAvailableValue').textContent=formatCents(p.freeAvailableCents); document.getElementById('freeAvailableValue').classList.toggle('negative',p.freeAvailableCents<0); const hero=document.querySelector('.hero-card'); if(hero)hero.classList.toggle('negative',p.freeAvailableCents<0);
  ...
  document.querySelectorAll('[data-donut-mode]').forEach(b=>{const active=b.dataset.donutMode===donutMode;b.classList.toggle('active',active);b.setAttribute('aria-selected',String(active));});
  ...
  budgetList.innerHTML=p.budgets.length?p.budgets.map(b=>{ const baseline=Math.max(0,b.plannedCents,b.spentCents+Math.max(0,b.availableCents));const ratio=baseline?Math.min(100,Math.max(0,(b.spentCents/baseline)*100)):0;const exceeded=b.availableCents<0;const status=exceeded?'Überschritten':ratio>=85?'Fast ausgeschöpft':'Im Rahmen';return `<button type="button" class="budget-card" data-budget-id="${escapeHtml(b.id)}"><div class="budget-top"><div><div class="budget-name">${escapeHtml(b.name)}</div><div class="budget-category">${escapeHtml(b.category)}</div></div><div class="budget-available ${exceeded?'negative':''}">${formatCents(b.availableCents)}<small>verfügbar</small></div></div><div class="progress" role="progressbar" aria-label="Budgetverbrauch ${escapeHtml(b.name)}" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(ratio)}"><span style="width:${ratio.toFixed(1)}%"></span></div><div class="budget-meta"><span>Ausgegeben ${formatCents(b.spentCents)}</span><span>Plan ${formatCents(b.plannedCents)}</span></div><div class="budget-status ${exceeded?'negative':''}">${status}</div></button>`}).join(''):'<div class="empty-inline">Keine Budgets im Plan-Code.</div>';
