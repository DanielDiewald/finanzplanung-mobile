...
export function renderTransactions({plan,transactions,filter='all',budgetFilter='',onEdit}){
  ...
  let rows=transactions.slice();if(filter==='expense')rows=rows.filter(t=>['budget_expense','expense'].includes(t.kind));else if(filter==='income')rows=rows.filter(t=>t.kind==='income');else if(filter==='pending')rows=rows.filter(t=>t.status!=='confirmed');
  ...
  list.innerHTML=groups.length?groups.map(([date,items])=>`<section class="transaction-group"><h3>${dayHeading(date)}</h3><div class="transaction-group-list">${items.map(t=>{
    if(t.kind==='capy_stash_deposit'){const reward=Math.max(0,Math.floor(Number(t.capyCoinReward)||0));return `<div class="transaction-row capy-transaction-row ${t.status==='rejected'?'is-rejected':''}"><span class="transaction-main"><strong>🔒 ${escapeHtml(plan?.capy?.budgetName||t.description||'Capy-Vorrat')}</strong><small>${escapeHtml(capyStatus(t))}${reward?` · +${reward} 🪙`:''}</small></span><span class="transaction-amount capy-transfer">${formatCents(t.amountCents)}</span></div>`;}
