...
async function shareTransactionCode(){ const code=$('transactionCodeText').value;if(!code||!navigator.share)return;try{await navigator.share({title:'Benedikt · Buchungen übertragen',text:code});}catch(err){if(err.name!=='AbortError')toast('Teilen fehlgeschlagen.',{error:true});} }
...
async function exportLocalBackup(){const data=await exportAllData();downloadText(`benedikt-backup-${todayLocal()}.json`,JSON.stringify(data,null,2));toast('Lokales Backup erstellt.');}
...
function setupEvents(){
  ...
  window.addEventListener('online',updateNetworkState);window.addEventListener('offline',updateNetworkState); document.addEventListener('visibilitychange',()=>{if(document.hidden)stopQrScanner($('scannerVideo'));}); const themeMedia=globalThis.matchMedia?.('(prefers-color-scheme: dark)'); themeMedia?.addEventListener?.('change',()=>{if(state.settings.theme==='system')applyTheme('system');});
